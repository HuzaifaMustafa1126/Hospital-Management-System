...
export const patientController = {
  async create(req, res) { const patient = await patientService.create(patientSchema.parse(patientInput(req.body)), req.user.id); await auditService.record({ userId: req.user.id, action: 'PATIENT_CREATED', entity: 'PATIENT', entityId: String(patient.id), newData: patient, ...context(req) }); res.status(201).json({ success: true, message: 'Patient registered successfully', data: patient }); },
