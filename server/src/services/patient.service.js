...
const normalizePhone = (value) => { const digits = String(value).replace(/\D/g, ''); if (/^03\d{9}$/.test(digits)) return `+92${digits.slice(1)}`; if (/^92\d{10}$/.test(digits)) return `+${digits}`; throw new AppError(400, 'Enter a valid Pakistani mobile number', [], 'phone'); };
...
async function findDuplicate(field, value, connection = database, excludeId = null) { const column = field === 'cnic' ? 'cnic' : 'phone'; const values = excludeId ? [value, excludeId] : [value]; const [rows] = await connection.execute(`SELECT id, patient_number AS patientNumber FROM patients WHERE ${column} = ? AND is_active = TRUE${excludeId ? ' AND id != ?' : ''} LIMIT 1`, values); return rows[0] || null; }

export const patientService = {
  normalizeCnic, normalizePhone,
  async create(data, actorId) {
    const cnic = normalizeCnic(data.cnic); const phone = normalizePhone(data.phone); const connection = await database.getConnection(); let locked = false;
    try {
      ...
      const cnicDuplicate = await findDuplicate('cnic', cnic, connection); if (cnicDuplicate) throw new AppError(409, 'A patient with this CNIC already exists.', [{ patientId: cnicDuplicate.id }], 'cnic');
      const phoneDuplicate = await findDuplicate('phone', phone, connection); if (phoneDuplicate) throw new AppError(409, 'A patient with this phone number already exists.', [{ patientId: phoneDuplicate.id }], 'phone');
      ...
      const [result] = await connection.execute('INSERT INTO patients (patient_number, first_name, last_name, father_name, cnic, phone, address, doctor_id, created_by, registration_locked) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, TRUE)', [patientNumber, data.firstName, data.lastName, data.fatherName, cnic, phone, data.address, data.doctorId, actorId]);
...
>>>>>>> 1b6046d (Add Departments)
  ...
  async list({ page, limit, search }) { const safePage = Number.isInteger(page) && page > 0 ? page : 1; const safeLimit = Number.isInteger(limit) && limit > 0 ? Math.min(limit, 100) : 20; const safeOffset = Math.max(0, (safePage - 1) * safeLimit); const term = search ? `%${search.trim()}%` : null; const where = term ? 'WHERE p.patient_number LIKE ? OR p.cnic LIKE ? OR p.phone LIKE ? OR CONCAT(p.first_name, \' \', p.last_name) LIKE ?' : ''; const params = term ? [term, term, term, term] : []; const [rows] = await database.execute(`${select} ${where} ORDER BY p.created_at DESC LIMIT ${safeLimit} OFFSET ${safeOffset}`, params); const [counts] = await database.execute(`SELECT COUNT(*) AS total FROM patients p ${where}`, params); const total = counts[0].total; return { items: rows.map(present), page: safePage, limit: safeLimit, total, totalPages: total ? Math.ceil(total / safeLimit) : 0 }; },
