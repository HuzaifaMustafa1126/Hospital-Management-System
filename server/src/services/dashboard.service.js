...
export const dashboardService = {
  async summary() {
    const [[patientCount], [todayCount], [doctorCount], [departmentCount], [activeServiceCount], [inactiveServiceCount], [recent], [trend], [revenue], [activity], [departmentActivity]] = await Promise.all([
      ...
      database.execute('SELECT COUNT(*) AS total FROM services WHERE is_active = TRUE'),
      database.execute('SELECT COUNT(*) AS total FROM services WHERE is_active = FALSE'),
      ...
      database.execute('SELECT d.name AS department, COUNT(s.id) AS activity FROM departments d LEFT JOIN services s ON s.department_id = d.id AND s.is_active = TRUE GROUP BY d.id, d.name ORDER BY d.name'),
    ]);
