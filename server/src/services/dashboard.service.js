...
export const dashboardService = {
  async summary() {
    const [[patientCount], [todayCount], [doctorCount], recent, trend, revenue, activity] = await Promise.all([
      ...
      database.execute("SELECT DATE_FORMAT(created_at, '%Y-%m-%d') AS date, COUNT(*) AS total FROM patients WHERE is_active = TRUE AND created_at >= CURDATE() - INTERVAL 6 DAY GROUP BY DATE(created_at) ORDER BY date"),
      database.execute("SELECT DATE_FORMAT(paid_at, '%Y-%m-%d') AS date, COALESCE(SUM(amount), 0) AS total FROM registration_payments WHERE payment_status = 'PAID' AND paid_at >= CURDATE() - INTERVAL 6 DAY GROUP BY DATE(paid_at) ORDER BY date"),
