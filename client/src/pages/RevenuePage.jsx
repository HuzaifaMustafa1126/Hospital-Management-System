...
export function RevenuePage() {
  ...
  return <div className="space-y-6"><section><p className="text-sm font-semibold text-teal-700">Super Admin</p><h2 className="mt-1 text-3xl font-bold">Revenue</h2><p className="mt-1 text-sm text-slate-500">Collected revenue is based on actual payments, not billed charges.</p></section><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{[["Today's Collected Revenue",money(data.todayRevenue),Banknote],["Today's Billed Amount",money(data.todayBilled),WalletCards],["Outstanding Balance",money(data.outstandingBalance),WalletCards],["Paid Bills",data.paidBills,Banknote],["Partially Paid Bills",data.partiallyPaidBills,WalletCards],["Unpaid Bills",data.unpaidBills,WalletCards]].map(([label,value,Icon])=><article className="hms-card p-5" key={label}><Icon className="text-teal-700" size={20}/><p className="mt-4 text-2xl font-bold">{value}</p><p className="text-sm text-slate-500">{label}</p></article>)}</section></div>;
}
