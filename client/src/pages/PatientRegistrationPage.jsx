...
export function PatientRegistrationPage() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    setError("");
    const amount = Number(form.registrationFee);
    if (
      !Object.entries(form)
        .filter(([k]) => [
          "firstName",
    ...
    if (
      ...
      (!Number.isFinite(amount) || amount < 0 || amount > Number(fee?.amount))
    )
      return setError(
        `Discounted registration fee must be between PKR 0 and PKR ${fee?.amount ?? 0}.`,
      );
    setSaving(true);
    try {
      const r = await patientService.create({
        ...
        registration_fee: form.feeType === "FREE" ? 0 : amount,
      });
      ...
      >
        ...
        <fieldset className="border-t pt-5 sm:col-span-2">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <label
              className={`cursor-pointer rounded-xl border p-4 ${form.feeType === "FREE" ? "border-teal-600 bg-teal-50" : "border-slate-200"}`}
            >
              <input
                className="sr-only"
                type="radio"
              ...
              <span className="mt-1 block text-sm text-slate-500">
                No registration fee · PKR 0
              </span>
            </label>
            <label
              className={`cursor-pointer rounded-xl border p-4 ${form.feeType === "DISCOUNTED" ? "border-teal-600 bg-teal-50" : "border-slate-200"}`}
            >
              <input
                className="sr-only"
                type="radio"
