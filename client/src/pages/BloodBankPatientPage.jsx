...
export function BloodBankPatientPage() {
  ...
  const [patient, setPatient] = useState(null),
    ...
    [visitId, setVisitId] = useState(""),
    [open, setOpen] = useState(false),
    [confirming, setConfirming] = useState(false),
    [form, setForm] = useState({ serviceId: "", quantity: 1, notes: "" }),
  ...
  const loadPatient = useCallback(async () => {
    try {
      const [patientResponse, servicesResponse] = await Promise.all([
        bloodBankService.getPatient(id),
        bloodBankService.availableServices(),
      ]);
      const loaded = patientResponse.data.data;
      setPatient(loaded);
      setServices(servicesResponse.data.data);
      setVisitId((current) => current || String(loaded.latestVisit?.id || ""));
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ||
          "Unable to load Blood Bank patient.",
      );
    }
  }, [id]);
  const loadHistory = useCallback(async (selectedVisitId) => {
    if (!selectedVisitId) {
      setHistory([]);
      return;
    }
    ...
    try {
      const response = await bloodBankService.visitServices(selectedVisitId);
      setHistory(response.data.data);
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ||
          "Unable to load Blood Bank history.",
      );
    } finally {
  ...
  const selected = services.find(
    (item) => String(item.id) === String(form.serviceId),
  );
  ...
  const review = (event) => {
    ...
    if (!visit || visit.status !== "OPEN")
      return setError(
        "No active visit found for this patient. Create a new visit at Reception first.",
      );
    if (!selected) return setError("Select a Blood Bank service.");
  ...
  const submit = async () => {
    setSaving(true);
    try {
      await bloodBankService.addService(visit.id, {
      ...
      notify({
        type: "success",
        title: "Blood Bank Service Added",
        message: `${selected.name} was added successfully.`,
      });
      setOpen(false);
      setConfirming(false);
      setForm({ serviceId: "", quantity: 1, notes: "" });
      setMessage("Blood Bank service added successfully.");
      setError("");
      await loadHistory(visit.id);
    } catch (requestError) {
      setError(
        requestError.response?.data?.message ||
          "Unable to add Blood Bank service.",
      );
      setConfirming(false);
    } finally {
      setSaving(false);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      <Link
        ...
        to="/blood-bank/patients"
      >
        <ArrowLeft size={16} /> Back to patient search
      </Link>
      {message && (
      ...
      <section className="hms-card p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row">
          ...
          {visit?.status === "OPEN" && (
            ...
            >
              <Plus size={16} /> Add Blood Bank Service
            </button>
          )}
        </div>
        ...
        <p className="mt-5 rounded-lg bg-slate-50 p-3 text-xs text-slate-500">
          Patient registration information is read-only.
        </p>
      </section>
      <section className="hms-card p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h3 className="font-bold">Visit Information</h3>
            <p className="text-sm text-slate-500">
              Select a visit to view isolated Blood Bank service history.
            </p>
          </div>
          <label className="text-sm font-semibold">
            Selected Visit
            <select
              className="hms-input min-w-56"
              value={visitId}
        ...
        {visit ? (
          <dl className="mt-5 grid gap-4 rounded-xl bg-slate-50 p-4 sm:grid-cols-4">
            <Info label="Visit Number" value={`Visit #${visit.visitNumber}`} />
            <Info
              label="Visit Date"
              value={new Date(visit.visitDate).toLocaleDateString()}
            />
            <Info label="Doctor" value={visit.doctorName} />
            <Info label="Visit Status" value={visit.status} />
          </dl>
        ) : (
          <p className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-amber-800">
            No active visit found for this patient. Create a new visit at
            Reception first.
          </p>
        )}
      </section>
      <section className="hms-card overflow-hidden">
        <div className="flex items-center gap-2 border-b border-slate-100 p-5">
          <Droplets className="text-rose-700" size={19} />
          <div>
            <h3 className="font-bold">Blood Bank Services</h3>
            <p className="text-sm text-slate-500">
              Only services belonging to the selected visit are shown.
            </p>
          </div>
        </div>
        {historyLoading ? (
              ...
              <tbody>
                {history.map((item) => (
                  <tr className="border-t border-slate-100" key={item.id}>
                    ...
                    <td className="px-5 py-4">{item.serviceCode}</td>
          ...
          <div className="p-10 text-center">
            <p className="text-sm text-slate-500">
              No Blood Bank services have been added for this visit.
            </p>
            {visit?.status === "OPEN" && (
              <button
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/55 p-4">
          ...
          >
            <button
              type="button"
              className="absolute right-5 top-5 p-1 text-slate-500"
              onClick={() => {
                setOpen(false);
                setConfirming(false);
              }}
            ...
            <h3 className="text-xl font-bold">Add Blood Bank Service</h3>
            <p className="mt-1 text-sm text-slate-500">
              The service price is loaded securely from the database.
            </p>
            {error && (
            ...
            <label className="mt-5 block text-sm font-semibold">
              ...
              >
                <option value="">Select Blood Bank service</option>
                {services.map((item) => (
                  <option value={item.id} key={item.id}>
                    {item.name} ({item.code}) — {money(item.price)}
                  </option>
                ))}
              </select>
            </label>
            ...
            {selected && (
              <div className="mt-5 flex items-center gap-3 rounded-xl bg-rose-50 p-4">
                <span className="rounded-xl bg-white p-2 text-rose-700">
                  {selected.icon === "test-tube" ? (
                    <TestTube size={20} />
                  ) : (
                    <Droplets size={20} />
                  )}
                ...
                <div className="ml-auto text-right">
                  ...
                  <p className="text-sm">Total: {money(total)}</p>
                </div>
              </div>
            )}
            {confirming && selected && (
              <div className="mt-5 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm">
                <h4 className="font-bold">Confirm Blood Bank Service</h4>
                <dl className="mt-3 grid grid-cols-2 gap-2">
                  ...
                  <dt>Visit</dt>
                  <dd className="font-semibold">Visit #{visit.visitNumber}</dd>
                  <dt>Service</dt>
