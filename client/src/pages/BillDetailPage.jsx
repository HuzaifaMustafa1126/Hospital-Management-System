...
export function BillDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setError(requestError.response?.data?.message || "Unable to load bill.");
    }
  }, [id]);
  useEffect(() => {
    void load();
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <section className="hms-card p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row">
          <div>
            <p className="text-sm font-semibold text-teal-700">Visit Bill</p>
            <h2 className="mt-1 text-2xl font-bold">{bill.billNumber}</h2>
            <p className="mt-1 text-sm text-slate-500">
              {bill.patientName} · {bill.patientNumber} · Visit #
              {bill.visitNumber}
            </p>
          </div>
          <span
            className={`h-fit rounded-full px-3 py-1.5 text-xs font-bold ${badge[bill.paymentStatus]}`}
          >
      ...
      <section className="hms-card p-6">
        ...
        <div className="mt-4 flex justify-between border-b border-slate-100 py-3 text-sm">
          <span>
            {bill.feeType === "FREE"
              ? "Free Visit Fee"
              : `${bill.feeType} Visit Fee`}
          </span>
          <b>{bill.feeType === "FREE" ? "FREE" : money(bill.visitFee)}</b>
        </div>
        {Object.entries(bill.serviceGroups).map(([department, services]) => (
          <div className="border-b border-slate-100 py-4" key={department}>
            <p className="hms-label">{department}</p>
            {services.map((service) => (
              ...
              >
                <span>
                  {service.serviceName}{" "}
                  <small className="text-slate-500">×{service.quantity}</small>
                </span>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/55 p-4">
          ...
          >
            ...
            <dl className="mt-4 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 p-4 text-sm">
              <dt>Patient</dt>
              <dd className="font-semibold">{bill.patientName}</dd>
              <dt>Visit</dt>
              <dd className="font-semibold">Visit #{bill.visitNumber}</dd>
              <dt>Bill</dt>
              <dd className="font-semibold">{bill.billNumber}</dd>
              <dt>Gross Total</dt>
              <dd className="font-semibold">{money(bill.grossTotal)}</dd>
              <dt>Already Paid</dt>
              <dd className="font-semibold">{money(bill.amountPaid)}</dd>
              <dt>Balance Due</dt>
              <dd className="font-semibold">{money(bill.balanceDue)}</dd>
            </dl>
            {error && (
