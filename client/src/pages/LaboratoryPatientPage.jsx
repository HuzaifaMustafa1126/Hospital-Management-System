...
export function LaboratoryPatientPage() {
  ...
  const [form, setForm] = useState({ serviceId: "", quantity: 1, notes: "" });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const load = async () => {
    try {
      const [p, s] = await Promise.all([
        laboratoryService.getPatient(id),
        laboratoryService.availableServices(),
      ]);
      setPatient(p.data.data);
      setAvailable(s.data.data);
    } catch (e) {
      setError(
        e.response?.data?.message || "Unable to load laboratory patient.",
      );
    }
  };
  useEffect(() => {
  ...
  const total = selected ? selected.price * Number(form.quantity || 0) : 0;
  const submit = async (e) => {
    e.preventDefault();
    setSaving(true);
    try {
      await laboratoryService.addService(id, {
      ...
      notify({
        type: "success",
        title: "Laboratory Service Added",
      ...
      setError(
        err.response?.data?.message || "Unable to add laboratory service.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      <section className="hms-card p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row">
          ...
          <button className="hms-button-primary" onClick={() => setOpen(true)}>
            <Plus size={16} /> Add laboratory service
          </button>
        </div>
      ...
      <section className="hms-card overflow-hidden">
        <div className="flex items-center gap-2 border-b border-slate-100 p-5">
          <FlaskConical className="text-teal-700" size={18} />
          <div>
            <h3 className="font-bold text-slate-900">
              Laboratory service history
            </h3>
            <p className="text-sm text-slate-500">
              Services added to this patient record.
            </p>
          </div>
        </div>
        {patient.laboratoryServices.length ? (
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {patient.laboratoryServices.map((s) => (
                  <tr className="border-t border-slate-100" key={s.id}>
                    <td className="px-5 py-4 font-medium text-slate-800">
                      {s.serviceName}
                    </td>
                    <td className="px-5 py-4">{s.quantity}</td>
                    <td className="px-5 py-4">{money(s.unitPrice)}</td>
                    <td className="px-5 py-4 font-semibold">
                      {money(s.totalAmount)}
                    </td>
                    <td className="px-5 py-4">{s.addedBy}</td>
          ...
          <p className="p-8 text-center text-sm text-slate-500">
            No laboratory services have been added for this patient.
          </p>
        )}
      </section>
      {open && (
          ...
          >
            ...
            {selected && (
              <div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm">
                ...
                <p>
                  <b>Service:</b> {selected.name}
                </p>
                <p>
                  <b>Unit price:</b> {money(selected.price)}
                </p>
