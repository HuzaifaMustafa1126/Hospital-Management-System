...
The React/Vite client owns presentation, routes, session context, and API services. The Express server owns validation, JWT authentication, authorization, business services, and parameterized SQL repositories. `server/src/db/database.js` provides the shared MySQL connection pool.
